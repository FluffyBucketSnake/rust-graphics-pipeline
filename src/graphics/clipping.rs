...
pub fn clip_line<V: Vertex>(line: Line<V>) -> Option<Line<V>> {
    ...
    loop {
        if (code0 | code1) == OutCode::INSIDE {
            ...
            // TODO: Fix regression that creates a floating point imprecision.
            let delta = e1 - e0;
            ...
            let intersection = V::interpolate(&e0, &e1, alpha);
...
pub fn clip_triangle<V: Vertex>(triangle: Triangle<V>) -> ClippedTriangle<V> {
    // Cull tests
...
fn clip_one_vertex<V: Vertex>(v0: V, v1: V, v2: V) -> ClippedTriangle<V> {
    ...
    let v0a = V::interpolate(&v0, &v1, alpha1);
    let v0b = V::interpolate(&v0, &v2, alpha2);

    ClippedTriangle::Two(Triangle(v0a, v1, v2), Triangle(v0b, v0a, v2))
}

/// Clips two vertices from the triangle. Creates only one triangle.
fn clip_two_vertices<V: Vertex>(v0: V, v1: V, v2: V) -> ClippedTriangle<V> {
    ...
    let v0 = V::interpolate(&v0, &v2, alpha1);
    let v1 = V::interpolate(&v1, &v2, alpha2);
